...
export function ThreadSidebar({
  ...
  return (
    <aside
      ...
      className="fixed top-0 left-0 flex h-screen flex-col overflow-hidden border-r border-[var(--cd-border)] bg-[var(--cd-surface)] transition-[width] duration-[250ms] ease-in-out"
    >
      {/* Header */}
      ...
      {!collapsed && (
        <div className="min-h-0 flex-1 overflow-y-auto px-2 py-2">
          {pinned.length > 0 && (
            <div className="mb-1">
              <p className="px-2.5 py-1.5 text-[11px] font-medium tracking-wider text-[var(--cd-text-tertiary)] uppercase">
                Pinned
              </p>
              {pinned.map((thread) => (
                <ThreadItem
